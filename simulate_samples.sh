...
#SBATCH --output=simulate.log
#SBATCH --error=simulate.err
#SBATCH --ntasks=1
...
do
	...
	if [ $launch_mode == 0 ]; then
        	srun ~/atlas/./atlas task=GLF bam=$BAM out=${newbam}
...
join -e 0  -a1 -a2 -1 2 -2 2 -o 1.1 0 $flds 2.{3..12} $file1 $file2 | awk '{$1 = "chr1"; print}' > ${runname}_chr20_atlas_merged.glf
...
if [ $(wc -l < ${runname}.filelist) -ge 3 ]; then
        ...
        do
                ...
                N=$(awk '{print NF}' $file1 | head -n 1)
                flds=$(eval echo 1.{3..$N})
                join -e 0  -a1 -a2 -1 2 -2 2 -o 1.1 0 $flds 2.{3..12} $file1 $file2 | awk '{$1 = "chr1"; print}' > atlas_merged_tmp.glf
...
srun angsd -glf10_text ${runname}_chr20_atlas_log.glf -fai ${runname}.fasta.fai -nInd ${nInd} -doSaf 1 -anc ${runname}.fasta -out ${runname}_atlas -P 4
srun realSFS ${runname}_atlas.saf.idx -maxIter 100 -P 4 > ${runname}.atlas.sfs

ls ${runname}*.bam > bam.filelist


#GL1
srun angsd -bam bam.filelist -GL 1 -nInd 10 -doSaf 1 -anc ${runname}.fasta -out ${runname}.samtools -P 4
srun realSFS ${runname}.samtools.saf.idx -maxIter 100 -P 4 > ${runname}.samtools.sfs
#GL2
srun angsd -bam bam.filelist -GL 2 -nInd 10 -doSaf 1 -anc ${runname}.fasta -out  ${runname}.gatk -P 4
